...
export default function BookingOperationsModal({
  openModal,
  setOpenModal,
  id,
  updeteBooking,
      ...
      {openModal && (
        <div className="fixed flex items-center justify-center dark:text-stone-100 h-screen w-screen z-[200] top-0 left-0 p-5">
          ...
          >
            <h1
              onClick={() => setOpenModal(false)}
              className="top-2 right-2 text-slate-400 absolute text-lg cursor-pointer"
            >
              <svg
                stroke="currentColor"
                fill="currentColor"
            ...
            <h1 className="delete-modal">
              {isDeleting ? "Delete booking" : "Check out booking"}
            </h1>
            ...
            <div className="flex gap-3 self-end text-[11px] mt-7 delete-modal">
              ...
              <Button
                ...
                bg={`${isDeleting ? "bg-red-700" : "bg-zinc-500"}`}
                hover={`${
                  isDeleting ? "hover:bg-red-800" : "hover:bg-zinc-600"
                }`}
              >
                {isDeleting ? "Delete" : "Check out"}
              </Button>
            </div>
          </motion.div>
        </div>
